...
async def lead_dashboard_filter_options():
    ...
    return {
        "pipelines": sorted(p for p in pipelines if p is not None),
        "utmSources": sorted(s for s in utm_sources if s not in (None, "")),
        "utmMediums": sorted(m for m in utm_mediums if m not in (None, "")),
    }
...
def _parse_int_csv(raw: Optional[str], field_name: str) -> list[int]:
    values = []
...
async def lead_dashboard(
    ...
    pipeline: Optional[int] = Query(None),
    ...
    examMode: Optional[int] = Query(None),
    ...
    Date/pipeline/utmSource/utmMedium run in the first $match so they can use
    the leads.createdAt/pipeline/utmSource/utmMedium compound index.
    ...
    utm_sources = _parse_csv(utmSource)
    utm_mediums = _parse_csv(utmMedium)
    ...
    if examMode is not None:
        post_match["customFieldValues.cfExamMode"] = examMode
    if course_codes:
